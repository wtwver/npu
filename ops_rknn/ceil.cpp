...
static std::string pick_model(int width) {
  std::string exact = "models/ceil_float16_1x" + std::to_string(width) + ".rknn";
...
static int run_case(int size) {
  ...
  for (int i = 0; i < total; i++) {
    __fp16 fp16 = static_cast<__fp16>(std::ceil(input_f[i]));
    expected_fp16[i] = static_cast<float>(fp16);
  }

  std::cout << "=== ceil " << size << "x" << size << " ===" << std::endl;
...
int main(int argc, char **argv) {
  ...
  for (int size : sizes) {
    ...
    if (ret == 2) {
      std::cout << "=== ceil " << size << "x" << size << " ===" << std::endl;
      std::cout << "SKIP: missing model ceil_float16_1x" << size << ".rknn" << std::endl;
      continue;
    }
    if (ret != 0) status = ret;
  }
