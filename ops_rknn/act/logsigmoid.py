"""
LogSigmoid LUT generator/approximation for RK3588-style NVDLA tables.
...
def logsigmoid_fn(x: float) -> float:
...
logsigmoid_lut, OUTPUT_SCALE = _generate_lut()
...
def hardware_logsigmoid(x_float: float) -> float:
    x_abs = abs(float(x_float))
    offset = _lut_offset(x_abs)
    lut_index = CENTER_INDEX + offset if x_float >= 0 else CENTER_INDEX - offset
    lut_raw = int(logsigmoid_lut[lut_index])
    return lut_raw / OUTPUT_SCALE


if __name__ == "__main__":
    sample_inputs = np.linspace(-6.0, 6.0, num=16, dtype=np.float32)
    expected = np.array([logsigmoid_fn(float(x)) for x in sample_inputs], dtype=np.float64)

    print(f"Generated LogSigmoid LUT with {len(logsigmoid_lut)} entries (OUTPUT_SCALE={OUTPUT_SCALE:.4f})")
    print("Testing hardware LogSigmoid approximation (NVDLA-style indexing):")
    ...
    for x, exp_val in zip(sample_inputs, expected):
        ...
        hw = hardware_logsigmoid(float(x))
        hw_vals.append(hw)
        print(
            f"x={x: .5f} idx={offset:3d} lut[{lut_idx}]=0x{int(logsigmoid_lut[lut_idx]) & 0xFFFF:04x} "
