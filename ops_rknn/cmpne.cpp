...
static std::string pick_model(int width) {
  std::string exact = "models/cmpne_float16_1x" + std::to_string(width) + ".rknn";
...
static int run_case(int size) {
  ...
  for (int i = 0; i < total; i++) {
    float vx = static_cast<float>((i % 13) - 6);
    float vy = static_cast<float>((i % 7) - 3);
    if ((i % 11) == 0) vy = vx;
  ...
  std::cout << "=== cmpne " << size << "x" << size << " (" << model_path << ") ===" << std::endl;
...
int main(int argc, char **argv) {
  ...
  for (int size : sizes) {
    ...
    if (ret == 2) {
      std::cout << "=== cmpne " << size << "x" << size << " ===" << std::endl;
      std::cout << "SKIP: missing model cmpne_float16_1x" << size << ".rknn" << std::endl;
      continue;
    }
    if (ret != 0) status = ret;
  }
