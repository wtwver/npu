...
static bool parse_shape(const std::string &shape_str, int dims[4]) {
  size_t start = 0;
  int idx = 0;
  while (start < shape_str.size() && idx < 4) {
    size_t end = shape_str.find('x', start);
    if (end == std::string::npos) end = shape_str.size();
    dims[idx++] = std::atoi(shape_str.substr(start, end - start).c_str());
    start = end + 1;
  }
  return idx == 4 && dims[0] > 0 && dims[1] > 0 && dims[2] > 0 && dims[3] > 0;
}

static std::string pick_model(int width) {
  std::string exact = "models/relu_float16_1x" + std::to_string(width) + ".rknn";
  if (file_exists(exact)) return exact;
  if (width == 1) return exact;  // still allow 1x1 fallback
...
int main(int argc, char **argv) {
  ...
  // shapes.push_back({1, 1, 2, 2});
  // shapes.push_back({1, 1, 4, 4});
  // shapes.push_back({1, 1, 5, 5});
  shapes.push_back({1, 1, 64, 64});
  ...
  for (auto dims : shapes) {
    ...
    for (int i = 0; i < total; i++) expected[i] = std::max(0.f, input_f[i]);
