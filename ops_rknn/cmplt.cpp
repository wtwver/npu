...
static std::string pick_model(int width) {
  std::string exact = "models/cmplt_float16_1x" + std::to_string(width) + ".rknn";
...
static int run_case(int size) {
  ...
  std::cout << "=== cmplt " << size << "x" << size << " (" << model_path << ") ===" << std::endl;
...
int main(int argc, char **argv) {
  ...
  for (int size : sizes) {
    ...
    if (ret == 2) {
      std::cout << "=== cmplt " << size << "x" << size << " ===" << std::endl;
      std::cout << "SKIP: missing model cmplt_float16_1x" << size << ".rknn" << std::endl;
      continue;
    }
    if (ret != 0) status = ret;
  }
