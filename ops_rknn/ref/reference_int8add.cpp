    ...
    int *type_bytes, int index)
{
    ...
    int8_t* test = (int8_t*)malloc(10 * sizeof(int8_t));
    ...
    int8_t* data = (int8_t*)malloc(10 * sizeof(int8_t));
    
    // Copy the data
    memcpy(data, test, 10 * sizeof(int8_t));
    
    printf("Input %d int8 data: ", index);
    for (int i = 0; i < 10; ++i) {
        printf("%d ", data[i]);
    }
    printf("\n");
...
int main() {
    ...
    for (int i = 0; i < io_num.n_input; i++) {
        input_data[i] = NULL;
        input_type[i] = RKNN_TENSOR_INT8;
        input_layout[i] = RKNN_TENSOR_UNDEFINED;
        input_size[i] = input_attrs[i].n_elems * sizeof(int8_t);
        type_bytes[i] = 4;
    }
