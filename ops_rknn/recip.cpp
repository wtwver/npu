...
static std::string pick_model(int width) {
  std::string exact = "models/recip_float16_1x" + std::to_string(width) + ".rknn";
...
static int run_case(int size) {
  ...
  if (!query_io(ctx, in_attrs, out_attrs) || in_attrs.size() < 2) {
    rknn_destroy(ctx);
    return -1;
  }

  std::vector<__fp16> input(total);
  std::vector<__fp16> ones(total);
  ...
  for (int i = 0; i < total; i++) {
    ...
    if (std::abs((float)hv) < 1e-3f) hv = static_cast<__fp16>(1.0f);
    input[i] = hv;
    ones[i] = static_cast<__fp16>(1.0f);
    input_f[i] = (float)hv;
  }
  ...
  in[0].buf = input.data();
  in[0].size = input.size() * sizeof(__fp16);

  in[1].index = 1;
  in[1].type = RKNN_TENSOR_FLOAT16;
  in[1].fmt = in_attrs[1].fmt;
  in[1].buf = ones.data();
  in[1].size = ones.size() * sizeof(__fp16);
...
int main(int argc, char **argv) {
  ...
  for (int size : sizes) {
    ...
    if (ret == 2) {
      std::cout << "=== recip " << size << "x" << size << " ===" << std::endl;
      std::cout << "SKIP: missing model recip_float16_1x" << size << ".rknn" << std::endl;
      continue;
    }
    if (ret != 0) status = ret;
  }
