...
static std::string pick_model(int width) {
  std::string exact = "models/tanh_float16_1x" + std::to_string(width) + ".rknn";
...
static int run_case(int width) {
  ...
  std::vector<__fp16> input(static_cast<size_t>(total));
  std::vector<float> input_f(static_cast<size_t>(total));
  Mt19937 rng{};
  mt_seed(&rng, 0);
  for (int64_t i = 0; i < total; i++) {
    float v = mt_uniform(&rng, -2.0f, 2.0f);
    __fp16 hv = static_cast<__fp16>(v);
    input[static_cast<size_t>(i)] = hv;
    input_f[static_cast<size_t>(i)] = static_cast<float>(hv);
  }
  ...
  for (int64_t i = 0; i < total; i++) {
    __fp16 fp16 = static_cast<__fp16>(std::tanh(input_f[static_cast<size_t>(i)]));
    expected_fp16[static_cast<size_t>(i)] = static_cast<float>(fp16);
  }

  std::cout << "=== tanh width " << width << " (elements " << total << ") ===" << std::endl;
...
int main(int argc, char **argv) {
  ...
  for (int width : widths) {
    ...
    if (ret == 2) {
      std::cout << "=== tanh width " << width << " ===" << std::endl;
      std::cout << "SKIP: missing model tanh_float16_1x" << width << ".rknn" << std::endl;
      continue;
    }
    if (ret != 0) status = ret;
  }
