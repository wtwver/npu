...
static std::string pick_model(int width) {
  std::string exact = "models/min_float16_1x" + std::to_string(width) + ".rknn";
...
static int run_case(int size) {
  ...
  for (int i = 0; i < total; i++) expected[i] = std::min(input_x_f[i], input_y_f[i]);

  std::cout << "=== min " << size << "x" << size << " (" << model_path << ") ===" << std::endl;
...
int main(int argc, char **argv) {
  ...
  for (int size : sizes) {
    ...
    if (ret == 2) {
      std::cout << "=== min " << size << "x" << size << " ===" << std::endl;
      std::cout << "SKIP: missing model min_float16_1x" << size << ".rknn" << std::endl;
      continue;
    }
    if (ret != 0) status = ret;
  }
