...
struct MemHandles createRegCmd(int fd, int type_size, uint32_t alu_algorithm)
{
    ...
    DPU_DATA_FORMAT_IN_PRECISION(0) |
    DPU_DATA_FORMAT_PROC_PRECISION(0));

    EMIT(REG_DPU_EW_CFG, DPU_EW_CFG_EW_CVT_TYPE(0) |
    DPU_EW_CFG_EW_DATA_MODE(1) |
    DPU_EW_CFG_EDATA_SIZE(1) |
    ...
    EMIT(REG_DPU_EW_CFG,
        DPU_EW_CFG_EW_CVT_TYPE(0) | DPU_EW_CFG_EW_DATA_MODE(1) |
           DPU_EW_CFG_EDATA_SIZE(1) | DPU_EW_CFG_EW_ALU_ALGO(current_alu_algorithm) |
    ...
    EMIT(REG_DPU_OUT_CVT_SCALE, 
        DPU_OUT_CVT_SCALE_FP32TOFP16_EN(1) |
        DPU_OUT_CVT_SCALE_OUT_CVT_SCALE(1)
        );
   ...
   EMIT(REG_DPU_RDMA_RDMA_DATA_CUBE_CHANNEL,
      ...
      EMIT(REG_DPU_RDMA_RDMA_ERDMA_CFG,
         DPU_RDMA_RDMA_ERDMA_CFG_ERDMA_DATA_MODE(1) |
            DPU_RDMA_RDMA_ERDMA_CFG_ERDMA_DATA_SIZE(1)); // 16 bit
      EMIT(REG_DPU_RDMA_RDMA_EW_SURF_STRIDE,
         DPU_RDMA_RDMA_EW_SURF_STRIDE_EW_SURF_STRIDE(1));
         //0x2001000178495044
         uint32_t rdma_feat_mode_cfg = 0x0;
         rdma_feat_mode_cfg |= DPU_RDMA_RDMA_FEATURE_MODE_CFG_IN_PRECISION(4);

         rdma_feat_mode_cfg |= DPU_RDMA_RDMA_FEATURE_MODE_CFG_BURST_LEN(15);
         rdma_feat_mode_cfg |= DPU_RDMA_RDMA_FEATURE_MODE_CFG_COMB_USE(4);
         rdma_feat_mode_cfg |= DPU_RDMA_RDMA_FEATURE_MODE_CFG_PROC_PRECISION(0);
         rdma_feat_mode_cfg |= DPU_RDMA_RDMA_FEATURE_MODE_CFG_MRDMA_DISABLE(0);
         rdma_feat_mode_cfg |= DPU_RDMA_RDMA_FEATURE_MODE_CFG_MRDMA_FP16TOFP32_EN(0);
         ...
         EMIT(REG_DPU_RDMA_RDMA_EW_SURF_NOTCH,
           DPU_RDMA_RDMA_EW_SURF_NOTCH_EW_SURF_NOTCH(0)); 
