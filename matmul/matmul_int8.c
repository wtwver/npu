 ...
 *
 * This file demonstrates the use of DRM_IOCTL_GEM_FLINK to create global names
 * for GEM objects, allowing them to be shared between processes. The flink names
 * can be used with DRM_IOCTL_GEM_OPEN to access the same memory from other processes.
 */
...
int main(int argc, char **argv) {
  ...
  if (create_flink_name(fd, regcmd_handle, &regcmd_flink) < 0) {
    printf("Failed to create flink name for regcmd\n");
    goto cleanup;
  }
  ...
  if (create_flink_name(fd, weights_handle, &weights_flink) < 0) {
    printf("Failed to create flink name for weights\n");
    goto cleanup;
  }
  ...
  if (open_gem_by_flink(fd, output_flink, &reopened_handle, &reopened_size) == 0) {
    ...
    if (map_ret >= 0) {
      ...
      if (reopened_map != MAP_FAILED) {
        ...
        int32_t *reopened_output = (int32_t*)reopened_map;
        printf("First few values from reopened output: %d, %d, %d\n",
