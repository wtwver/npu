...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3) {
        ...
        std::cerr << "Operations: mul, add, sub" << std::endl;
        return -1;
    }
    
    // Parse operation from command line argument
    std::string operation = argv[1];
    if (operation != "mul" && operation != "add" && operation != "sub") {
        std::cerr << "Error: Operation must be either 'mul', 'add', or 'sub'" << std::endl;
        return -1;
    }
    ...
    // Print inputs with column alignment
    std::cout << "Input0  : ";
    for (int i = 0; i < size; i++) {
        if (i > 0) std::cout << " ";
        std::cout.width(2);
    ...
    // Print results with column alignment
    std::cout << "Results : ";
    for (int i = 0; i < size; i++) {
        if (i > 0) std::cout << " ";
        std::cout.width(2);
    ...
    // Show expected results with column alignment
    std::cout << "Expected: ";
    if (operation == "mul") {
        for (int i = 0; i < size; i++) {
            if (i > 0) std::cout << " ";
