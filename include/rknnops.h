...
static void pack_conv_weights_fp16(__fp16 *dst, const __fp16 *src,
   ...
   for (int oc = 0; oc < out_channels; oc++) {
      ...
      int kh_src = (oc / kernel_w) % kernel_h;  // reuse the same kernel row per OC tile
      for (int kh = 0; kh < kernel_h; kh++) {
         for (int kw = 0; kw < kernel_w; kw++) {
            size_t dst_spatial_base = dst_kernel_base + ((size_t)kh * kernel_w + kw) * c2_out;
            for (int ic = 0; ic < in_channels; ic++) {
               size_t src_idx = (((size_t)oc * in_channels + ic) * kernel_h + kh_src) * kernel_w + kw;
...
void regcmd_helper(input_dma, weights_dma, output_dma){
   ...
   for (int i = 0; i < 1; ++i) {
      if (current_alu_algorithm == 13) { //CONV2d
         ...
         EMIT(REG_CNA_DATA_SIZE0, CNA_DATA_SIZE0_DATAIN_WIDTH(8) | CNA_DATA_SIZE0_DATAIN_HEIGHT(5));
         EMIT(REG_CNA_DATA_SIZE1, CNA_DATA_SIZE1_DATAIN_CHANNEL_REAL(2) | CNA_DATA_SIZE1_DATAIN_CHANNEL(8));
         EMIT(REG_CNA_DATA_SIZE2, CNA_DATA_SIZE2_DATAOUT_WIDTH(5));
         EMIT(REG_CNA_DATA_SIZE3, CNA_DATA_SIZE3_DATAOUT_ATOMICS(20));
         EMIT(REG_CNA_WEIGHT_SIZE0, 0x00000240);
         EMIT(REG_CNA_WEIGHT_SIZE1, CNA_WEIGHT_SIZE1_WEIGHT_BYTES_PER_KERNEL(96));
         EMIT(REG_CNA_WEIGHT_SIZE2, CNA_WEIGHT_SIZE2_WEIGHT_WIDTH(3) | CNA_WEIGHT_SIZE2_WEIGHT_HEIGHT(2) | CNA_WEIGHT_SIZE2_WEIGHT_KERNELS(6));
         EMIT(REG_CNA_CBUF_CON0, CNA_CBUF_CON0_WEIGHT_BANK(11) | CNA_CBUF_CON0_DATA_BANK(1));
         EMIT(REG_CNA_CBUF_CON1, CNA_CBUF_CON1_DATA_ENTRIES(40));
         ...
         EMIT(REG_CNA_DMA_CON2, CNA_DMA_CON2_SURF_STRIDE(32));
         EMIT(REG_CNA_FC_DATA_SIZE0, CNA_FC_DATA_SIZE0_DMA_WIDTH(7) | CNA_FC_DATA_SIZE0_DMA_HEIGHT(5));
         ...
         EMIT(REG_CORE_DATAOUT_SIZE_0, CORE_DATAOUT_SIZE_0_DATAOUT_HEIGHT(3) | CORE_DATAOUT_SIZE_0_DATAOUT_WIDTH(4));
         EMIT(REG_CORE_DATAOUT_SIZE_1, CORE_DATAOUT_SIZE_1_DATAOUT_CHANNEL(15));
         EMIT(REG_CORE_CLIP_TRUNCATE, 0x00000000);
         // [ffef0c08] lsb 0801000000003030 - CORE Unknown
         ...
         EMIT(REG_DPU_DST_SURF_STRIDE, DPU_DST_SURF_STRIDE_DST_SURF_STRIDE(20));
         EMIT(REG_DPU_DATA_CUBE_WIDTH, DPU_DATA_CUBE_WIDTH_WIDTH(4));
         ...
         EMIT(REG_DPU_WDMA_SIZE_1, DPU_WDMA_SIZE_1_HEIGHT_WDMA(3) | DPU_WDMA_SIZE_1_WIDTH_WDMA(4));
         ...
         EMIT(REG_DPU_SURFACE_ADD, DPU_SURFACE_ADD_SURF_ADD(40));
         // [ffef0d38] lsb 10010000000040c4 - DPU Unknown
         ...
         if (false){
            EMIT(REG_DPU_S_POINTER, DPU_S_POINTER_POINTER_PP_MODE(1) | DPU_S_POINTER_EXECUTER_PP_EN(1) | DPU_S_POINTER_POINTER_PP_EN(1));
            EMIT(REG_CNA_CONV_CON1, CNA_CONV_CON1_NONALIGN_DMA(1) | CNA_CONV_CON1_GROUP_LINE_OFF(1) | CNA_CONV_CON1_ARGB_IN(10) | CNA_CONV_CON1_PROC_PRECISION(2) | CNA_CONV_CON1_IN_PRECISION(2));
            EMIT(REG_CNA_CONV_CON2, CNA_CONV_CON2_RESERVED_0(16) | CNA_CONV_CON2_FEATURE_GRAINS(5));
            EMIT(REG_CNA_CONV_CON3, CNA_CONV_CON3_CONV_Y_STRIDE(1) | CNA_CONV_CON3_CONV_X_STRIDE(1));
            EMIT(REG_CNA_DATA_SIZE0, CNA_DATA_SIZE0_DATAIN_WIDTH(8) | CNA_DATA_SIZE0_DATAIN_HEIGHT(3));
            EMIT(REG_CNA_DATA_SIZE1, CNA_DATA_SIZE1_DATAIN_CHANNEL_REAL(2) | CNA_DATA_SIZE1_DATAIN_CHANNEL(8));
            EMIT(REG_CNA_DATA_SIZE2, CNA_DATA_SIZE2_DATAOUT_WIDTH(5));
            EMIT(REG_CNA_DATA_SIZE3, CNA_DATA_SIZE3_DATAOUT_ATOMICS(10));
            EMIT(REG_CNA_WEIGHT_SIZE0, 0x00000240);
            EMIT(REG_CNA_WEIGHT_SIZE1, CNA_WEIGHT_SIZE1_WEIGHT_BYTES_PER_KERNEL(96));
            EMIT(REG_CNA_WEIGHT_SIZE2, CNA_WEIGHT_SIZE2_WEIGHT_WIDTH(3) | CNA_WEIGHT_SIZE2_WEIGHT_HEIGHT(2) | CNA_WEIGHT_SIZE2_WEIGHT_KERNELS(6));
            EMIT(REG_CNA_CBUF_CON0, CNA_CBUF_CON0_WEIGHT_BANK(11) | CNA_CBUF_CON0_DATA_BANK(1));
            ...
            EMIT(REG_CNA_DMA_CON1, CNA_DMA_CON1_LINE_STRIDE(8));
            EMIT(REG_CNA_DMA_CON2, CNA_DMA_CON2_SURF_STRIDE(32));
            EMIT(REG_CNA_FC_DATA_SIZE0, CNA_FC_DATA_SIZE0_DMA_WIDTH(7) | CNA_FC_DATA_SIZE0_DMA_HEIGHT(3));
            EMIT(REG_CNA_FC_DATA_SIZE1, CNA_FC_DATA_SIZE1_DMA_CHANNEL(8));
            EMIT(REG_CNA_DCOMP_ADDR0, CNA_DCOMP_ADDR0_DECOMPRESS_ADDR0(weights_dma));
            EMIT(REG_CNA_CVT_CON5, 0x00000fff);
            EMIT(REG_CORE_MISC_CFG, CORE_MISC_CFG_PROC_PRECISION(2));
            EMIT(REG_CORE_DATAOUT_SIZE_0, CORE_DATAOUT_SIZE_0_DATAOUT_HEIGHT(1) | CORE_DATAOUT_SIZE_0_DATAOUT_WIDTH(4));
            EMIT(REG_CORE_DATAOUT_SIZE_1, CORE_DATAOUT_SIZE_1_DATAOUT_CHANNEL(15));
            // [ffef0f68] lsb 0801000000003030 - CORE Unknown
            emit_raw(&regs, CORE | 0x1, 0x3030, 0);
            EMIT(REG_DPU_FEATURE_MODE_CFG, DPU_FEATURE_MODE_CFG_BURST_LEN(15) | DPU_FEATURE_MODE_CFG_OUTPUT_MODE(2));
            EMIT(REG_DPU_DATA_FORMAT, DPU_DATA_FORMAT_OUT_PRECISION(2) | DPU_DATA_FORMAT_IN_PRECISION(2) | DPU_DATA_FORMAT_PROC_PRECISION(2));
            EMIT(REG_DPU_DST_BASE_ADDR, DPU_DST_BASE_ADDR_DST_BASE_ADDR(output_dma));
            ...
            EMIT(REG_DPU_BS_OW_CFG, DPU_BS_OW_CFG_SIZE_E_2(1) | DPU_BS_OW_CFG_SIZE_E_1(1) | DPU_BS_OW_CFG_SIZE_E_0(1) | DPU_BS_OW_CFG_OD_BYPASS(1));
            EMIT(REG_DPU_WDMA_SIZE_0, DPU_WDMA_SIZE_0_CHANNEL_WDMA(15));
            EMIT(REG_DPU_WDMA_SIZE_1, DPU_WDMA_SIZE_1_HEIGHT_WDMA(1) | DPU_WDMA_SIZE_1_WIDTH_WDMA(4));
            EMIT(REG_DPU_BN_CFG, DPU_BN_CFG_BN_RELU_BYPASS(1) | DPU_BN_CFG_BN_MUL_BYPASS(1) | DPU_BN_CFG_BN_ALU_BYPASS(1) | DPU_BN_CFG_BN_BYPASS(1));
            EMIT(REG_DPU_EW_CFG, DPU_EW_CFG_EW_RELU_BYPASS(1) | DPU_EW_CFG_EW_OP_CVT_BYPASS(1) | DPU_EW_CFG_EW_LUT_BYPASS(1) | DPU_EW_CFG_EW_OP_BYPASS(1) | DPU_EW_CFG_EW_BYPASS(1));
            EMIT(REG_DPU_EW_CVT_SCALE_VALUE, DPU_EW_CVT_SCALE_VALUE_EW_OP_CVT_SCALE(1));
            EMIT(REG_DPU_OUT_CVT_SCALE, DPU_OUT_CVT_SCALE_FP32TOFP16_EN(1) | DPU_OUT_CVT_SCALE_OUT_CVT_SCALE(1));
            EMIT(REG_DPU_SURFACE_ADD, DPU_SURFACE_ADD_SURF_ADD(40));
            // [ffef1098] lsb 10010000000040c4 - DPU Unknown
            emit_raw(&regs, 0x0 | 0x1, 0x40c4, 0);
            // EMIT(REG_PC_BASE_ADDRESS, PC_BASE_ADDRESS_PC_SOURCE_ADDR(0  ));
            EMIT(REG_PC_REGISTER_AMOUNTS, PC_REGISTER_AMOUNTS_RESERVED_0(1) | PC_REGISTER_AMOUNTS_PC_DATA_AMOUNT(7));
            emit_raw(&regs, 0x81, REG_PC_OPERATION_ENABLE, PC_OPERATION_ENABLE_RESERVED_0(6) | PC_OPERATION_ENABLE_OP_EN(1));
            finish_current_task();
         }
         ...
         // EMIT(REG_PPU_DST_BASE_ADDR, PPU_DST_BASE_ADDR_DST_BASE_ADDR((weights_dma+0x240)/16));
         ...
         // EMIT(REG_PPU_RDMA_RDMA_SRC_BASE_ADDR, (weights_dma+0x640)/16);
         ...
         // EMIT(REG_CNA_CVT_CON3, CNA_CVT_CON3_CVT_SCALE2(1));
         // EMIT(REG_CNA_CVT_CON4, CNA_CVT_CON4_CVT_SCALE3(1));
         // EMIT(REG_CNA_FEATURE_DATA_ADDR, CNA_FEATURE_DATA_ADDR_FEATURE_BASE_ADDR(input_dma)+0x60);
         // EMIT(REG_CNA_DMA_CON0, CNA_DMA_CON0_WEIGHT_BURST_LEN(15) | CNA_DMA_CON0_DATA_BURST_LEN(15));
         // EMIT(REG_CNA_DMA_CON1, CNA_DMA_CON1_LINE_STRIDE(8));
         // EMIT(REG_CNA_DMA_CON2, CNA_DMA_CON2_SURF_STRIDE(32));
         // EMIT(REG_CNA_FC_DATA_SIZE0, CNA_FC_DATA_SIZE0_DMA_WIDTH(7) | CNA_FC_DATA_SIZE0_DMA_HEIGHT(3));
         ...
         // EMIT(REG_CORE_DATAOUT_SIZE_0, CORE_DATAOUT_SIZE_0_DATAOUT_HEIGHT(1) | CORE_DATAOUT_SIZE_0_DATAOUT_WIDTH(4));
         // EMIT(REG_CORE_DATAOUT_SIZE_1, CORE_DATAOUT_SIZE_1_DATAOUT_CHANNEL(15));
         // // [ffef13e8] lsb 0801000000003030 - CORE Unknown
         // emit_raw(&regs, CORE | 0x1, 0x3030, 0);
         // EMIT(REG_DPU_FEATURE_MODE_CFG, DPU_FEATURE_MODE_CFG_BURST_LEN(15) | DPU_FEATURE_MODE_CFG_OUTPUT_MODE(2));
         // EMIT(REG_DPU_DATA_FORMAT, DPU_DATA_FORMAT_OUT_PRECISION(2) | DPU_DATA_FORMAT_IN_PRECISION(2) | DPU_DATA_FORMAT_PROC_PRECISION(2));
         // EMIT(REG_DPU_DST_BASE_ADDR, DPU_DST_BASE_ADDR_DST_BASE_ADDR(output_dma)+0xa0);
         // EMIT(REG_DPU_DST_SURF_STRIDE, DPU_DST_SURF_STRIDE_DST_SURF_STRIDE(20));
         // EMIT(REG_DPU_DATA_CUBE_WIDTH, DPU_DATA_CUBE_WIDTH_WIDTH(4));
         // EMIT(REG_DPU_DATA_CUBE_HEIGHT, DPU_DATA_CUBE_HEIGHT_HEIGHT(1));
         // EMIT(REG_DPU_DATA_CUBE_CHANNEL, DPU_DATA_CUBE_CHANNEL_ORIG_CHANNEL(5) | DPU_DATA_CUBE_CHANNEL_CHANNEL(15));
         ...
         // EMIT(REG_DPU_OUT_CVT_SCALE, DPU_OUT_CVT_SCALE_FP32TOFP16_EN(1) | DPU_OUT_CVT_SCALE_OUT_CVT_SCALE(1));
         // EMIT(REG_DPU_SURFACE_ADD, DPU_SURFACE_ADD_SURF_ADD(40));
         // // [ffef1518] lsb 10010000000040c4 - DPU Unknown
         // emit_raw(&regs, 0x0 | 0x1, 0x40c4, 0);
         // EMIT(REG_PC_BASE_ADDRESS, PC_BASE_ADDRESS_PC_SOURCE_ADDR(0));
         ...
         // EMIT(REG_PPU_DATA_CUBE_OUT_CHANNEL, PPU_DATA_CUBE_OUT_CHANNEL_CUBE_OUT_CHANNEL(31));
         // EMIT(REG_PPU_OPERATION_MODE_CFG, PPU_OPERATION_MODE_CFG_FLYING_MODE(1) | PPU_OPERATION_MODE_CFG_POOLING_METHOD(1));
         // EMIT(REG_PPU_DST_BASE_ADDR, PPU_DST_BASE_ADDR_DST_BASE_ADDR(268425252));
         // EMIT(REG_PPU_DST_SURF_STRIDE, PPU_DST_SURF_STRIDE_DST_SURF_STRIDE(1));
         // EMIT(REG_PPU_DATA_FORMAT, PPU_DATA_FORMAT_INDEX_ADD(1));
         // EMIT(REG_PPU_MISC_CTRL, PPU_MISC_CTRL_BURST_LEN(3));
         // EMIT(REG_PPU_RDMA_RDMA_CUBE_IN_CHANNEL, PPU_RDMA_RDMA_CUBE_IN_CHANNEL_CUBE_IN_CHANNEL(31));
         // // Offset 0x4640 matches RKNN conv2d intermediate buffer layout
         // EMIT(REG_PPU_RDMA_RDMA_SRC_BASE_ADDR, PPU_RDMA_RDMA_SRC_BASE_ADDR_SRC_BASE_ADDR((uint32_t)(output_dma + 0x4640)));
         // EMIT(REG_PPU_RDMA_RDMA_SRC_LINE_STRIDE, PPU_RDMA_RDMA_SRC_LINE_STRIDE_SRC_LINE_STRIDE(1));
         // EMIT(REG_PPU_RDMA_RDMA_SRC_SURF_STRIDE, PPU_RDMA_RDMA_SRC_SURF_STRIDE_SRC_SURF_STRIDE(1));
         // EMIT(REG_PPU_RDMA_RDMA_DATA_FORMAT, PPU_RDMA_RDMA_DATA_FORMAT_IN_PRECISION(1));
         // EMIT(REG_PC_OPERATION_ENABLE, PC_OPERATION_ENABLE_RESERVED_0(48));
         ...
         // EMIT(REG_DPU_S_POINTER, DPU_S_POINTER_POINTER_PP_MODE(1) | DPU_S_POINTER_EXECUTER_PP_EN(1) | DPU_S_POINTER_POINTER_PP_EN(1));
         // EMIT(REG_CNA_CONV_CON1, CNA_CONV_CON1_NONALIGN_DMA(1) | CNA_CONV_CON1_GROUP_LINE_OFF(1) | CNA_CONV_CON1_ARGB_IN(10) | CNA_CONV_CON1_PROC_PRECISION(2) | CNA_CONV_CON1_IN_PRECISION(2));
         // EMIT(REG_CNA_CONV_CON2, CNA_CONV_CON2_RESERVED_0(32) | CNA_CONV_CON2_FEATURE_GRAINS(5));
         // EMIT(REG_CNA_CONV_CON3, CNA_CONV_CON3_CONV_Y_STRIDE(1) | CNA_CONV_CON3_CONV_X_STRIDE(1));
         // EMIT(REG_CNA_DATA_SIZE0, CNA_DATA_SIZE0_DATAIN_WIDTH(8) | CNA_DATA_SIZE0_DATAIN_HEIGHT(3));
         // EMIT(REG_CNA_DATA_SIZE1, CNA_DATA_SIZE1_DATAIN_CHANNEL_REAL(2) | CNA_DATA_SIZE1_DATAIN_CHANNEL(8));
         // EMIT(REG_CNA_DATA_SIZE2, CNA_DATA_SIZE2_DATAOUT_WIDTH(5));
         ...
         // EMIT(REG_CORE_DATAOUT_SIZE_0, CORE_DATAOUT_SIZE_0_DATAOUT_HEIGHT(1) | CORE_DATAOUT_SIZE_0_DATAOUT_WIDTH(4));
         // EMIT(REG_CORE_DATAOUT_SIZE_1, CORE_DATAOUT_SIZE_1_DATAOUT_CHANNEL(15));
         // // [ffef1868] lsb 0801000000003030 - CORE Unknown
         // emit_raw(&regs, CORE | 0x1, 0x3030, 0);
         // EMIT(REG_DPU_FEATURE_MODE_CFG, DPU_FEATURE_MODE_CFG_BURST_LEN(15) | DPU_FEATURE_MODE_CFG_OUTPUT_MODE(2));
         // EMIT(REG_DPU_DATA_FORMAT, DPU_DATA_FORMAT_OUT_PRECISION(2) | DPU_DATA_FORMAT_IN_PRECISION(2) | DPU_DATA_FORMAT_PROC_PRECISION(2));
         // EMIT(REG_DPU_DST_BASE_ADDR, DPU_DST_BASE_ADDR_DST_BASE_ADDR(output_dma));
         ...
         // EMIT(REG_DPU_OUT_CVT_SCALE, DPU_OUT_CVT_SCALE_FP32TOFP16_EN(1) | DPU_OUT_CVT_SCALE_OUT_CVT_SCALE(1));
         // EMIT(REG_DPU_SURFACE_ADD, DPU_SURFACE_ADD_SURF_ADD(40));
         // // [ffef1998] lsb 10010000000040c4 - DPU Unknown
         // emit_raw(&regs, 0x0 | 0x1, 0x40c4, 0);
         // EMIT(REG_PC_BASE_ADDRESS, PC_BASE_ADDRESS_PC_SOURCE_ADDR(0));
         // EMIT(REG_PC_REGISTER_AMOUNTS, PC_REGISTER_AMOUNTS_RESERVED_0(2) | PC_REGISTER_AMOUNTS_PC_DATA_AMOUNT(14));
         // EMIT(REG_PC_OPERATION_ENABLE, PC_OPERATION_ENABLE_RESERVED_0(6) | PC_OPERATION_ENABLE_OP_EN(1));
         // finish_current_task();
         ...
         // EMIT(REG_DPU_S_POINTER, DPU_S_POINTER_POINTER_PP_MODE(1) | DPU_S_POINTER_EXECUTER_PP_EN(1) | DPU_S_POINTER_POINTER_PP_EN(1));
         // EMIT(REG_CNA_CONV_CON1, CNA_CONV_CON1_NONALIGN_DMA(1) | CNA_CONV_CON1_GROUP_LINE_OFF(1) | CNA_CONV_CON1_ARGB_IN(10) | CNA_CONV_CON1_PROC_PRECISION(2) | CNA_CONV_CON1_IN_PRECISION(2));
         // EMIT(REG_CNA_CONV_CON2, CNA_CONV_CON2_RESERVED_0(32) | CNA_CONV_CON2_FEATURE_GRAINS(4));
         // EMIT(REG_CNA_CONV_CON3, CNA_CONV_CON3_CONV_Y_STRIDE(1) | CNA_CONV_CON3_CONV_X_STRIDE(1));
         // EMIT(REG_CNA_DATA_SIZE0, CNA_DATA_SIZE0_DATAIN_WIDTH(8) | CNA_DATA_SIZE0_DATAIN_HEIGHT(2));
         // EMIT(REG_CNA_DATA_SIZE1, CNA_DATA_SIZE1_DATAIN_CHANNEL_REAL(2) | CNA_DATA_SIZE1_DATAIN_CHANNEL(8));
         // EMIT(REG_CNA_DATA_SIZE2, CNA_DATA_SIZE2_DATAOUT_WIDTH(5));
         // EMIT(REG_CNA_DATA_SIZE3, CNA_DATA_SIZE3_DATAOUT_ATOMICS(5));
         // EMIT(REG_CNA_WEIGHT_SIZE0, 0x00000240);
         // EMIT(REG_CNA_WEIGHT_SIZE1, CNA_WEIGHT_SIZE1_WEIGHT_BYTES_PER_KERNEL(96));
         // EMIT(REG_CNA_WEIGHT_SIZE2, CNA_WEIGHT_SIZE2_WEIGHT_WIDTH(3) | CNA_WEIGHT_SIZE2_WEIGHT_HEIGHT(2) | CNA_WEIGHT_SIZE2_WEIGHT_KERNELS(6));
         // EMIT(REG_CNA_CBUF_CON0, CNA_CBUF_CON0_WEIGHT_BANK(11) | CNA_CBUF_CON0_DATA_BANK(1));
         ...
         // EMIT(REG_CNA_DMA_CON1, CNA_DMA_CON1_LINE_STRIDE(8));
         // EMIT(REG_CNA_DMA_CON2, CNA_DMA_CON2_SURF_STRIDE(32));
         // EMIT(REG_CNA_FC_DATA_SIZE0, CNA_FC_DATA_SIZE0_DMA_WIDTH(7) | CNA_FC_DATA_SIZE0_DMA_HEIGHT(2));
         // EMIT(REG_CNA_FC_DATA_SIZE1, CNA_FC_DATA_SIZE1_DMA_CHANNEL(8));
         // EMIT(REG_CNA_DCOMP_ADDR0, CNA_DCOMP_ADDR0_DECOMPRESS_ADDR0(weights_dma));
         // EMIT(REG_CNA_CVT_CON5, 0x00000fff);
         // EMIT(REG_CORE_MISC_CFG, CORE_MISC_CFG_PROC_PRECISION(2));
         // EMIT(REG_CORE_DATAOUT_SIZE_0, CORE_DATAOUT_SIZE_0_DATAOUT_WIDTH(4));
         // EMIT(REG_CORE_DATAOUT_SIZE_1, CORE_DATAOUT_SIZE_1_DATAOUT_CHANNEL(15));
         // // [ffef1ce8] lsb 0801000000003030 - CORE Unknown
         // emit_raw(&regs, CORE | 0x1, 0x3030, 0);
         // EMIT(REG_DPU_FEATURE_MODE_CFG, DPU_FEATURE_MODE_CFG_BURST_LEN(15) | DPU_FEATURE_MODE_CFG_OUTPUT_MODE(2));
         // EMIT(REG_DPU_DATA_FORMAT, DPU_DATA_FORMAT_OUT_PRECISION(2) | DPU_DATA_FORMAT_IN_PRECISION(2) | DPU_DATA_FORMAT_PROC_PRECISION(2));
         // EMIT(REG_DPU_DST_BASE_ADDR, DPU_DST_BASE_ADDR_DST_BASE_ADDR(output_dma)+0xa0);
         ...
         // EMIT(REG_DPU_BS_OW_CFG, DPU_BS_OW_CFG_SIZE_E_2(1) | DPU_BS_OW_CFG_SIZE_E_1(1) | DPU_BS_OW_CFG_SIZE_E_0(1) | DPU_BS_OW_CFG_OD_BYPASS(1));
         // EMIT(REG_DPU_WDMA_SIZE_0, DPU_WDMA_SIZE_0_CHANNEL_WDMA(15));
         // EMIT(REG_DPU_WDMA_SIZE_1, DPU_WDMA_SIZE_1_WIDTH_WDMA(4));
         // EMIT(REG_DPU_BN_CFG, DPU_BN_CFG_BN_RELU_BYPASS(1) | DPU_BN_CFG_BN_MUL_BYPASS(1) | DPU_BN_CFG_BN_ALU_BYPASS(1) | DPU_BN_CFG_BN_BYPASS(1));
         // EMIT(REG_DPU_EW_CFG, DPU_EW_CFG_EW_RELU_BYPASS(1) | DPU_EW_CFG_EW_OP_CVT_BYPASS(1) | DPU_EW_CFG_EW_LUT_BYPASS(1) | DPU_EW_CFG_EW_OP_BYPASS(1) | DPU_EW_CFG_EW_BYPASS(1));
         // EMIT(REG_DPU_EW_CVT_SCALE_VALUE, DPU_EW_CVT_SCALE_VALUE_EW_OP_CVT_SCALE(1));
         // EMIT(REG_DPU_OUT_CVT_SCALE, DPU_OUT_CVT_SCALE_FP32TOFP16_EN(1) | DPU_OUT_CVT_SCALE_OUT_CVT_SCALE(1));
         // EMIT(REG_DPU_SURFACE_ADD, DPU_SURFACE_ADD_SURF_ADD(40));
         // // [ffef1e18] lsb 10010000000040c4 - DPU Unknown
         // emit_raw(&regs, 0x0 | 0x1, 0x40c4, 0);
         // EMIT(REG_PC_BASE_ADDRESS, PC_BASE_ADDRESS_PC_SOURCE_ADDR(0));
         // EMIT(REG_PC_REGISTER_AMOUNTS, PC_REGISTER_AMOUNTS_RESERVED_0(2) | PC_REGISTER_AMOUNTS_PC_DATA_AMOUNT(14));
         // EMIT(REG_PC_OPERATION_ENABLE, PC_OPERATION_ENABLE_RESERVED_0(6) | PC_OPERATION_ENABLE_OP_EN(1));
         ...
         // EMIT(REG_CNA_CVT_CON3, CNA_CVT_CON3_CVT_SCALE2(1));
         // EMIT(REG_CNA_CVT_CON4, CNA_CVT_CON4_CVT_SCALE3(1));
         // EMIT(REG_CNA_FEATURE_DATA_ADDR, CNA_FEATURE_DATA_ADDR_FEATURE_BASE_ADDR(input_dma)+0x90);
         // EMIT(REG_CNA_DMA_CON0, CNA_DMA_CON0_WEIGHT_BURST_LEN(15) | CNA_DMA_CON0_DATA_BURST_LEN(15));
         // EMIT(REG_CNA_DMA_CON1, CNA_DMA_CON1_LINE_STRIDE(8));
         // EMIT(REG_CNA_DMA_CON2, CNA_DMA_CON2_SURF_STRIDE(32));
         // EMIT(REG_CNA_FC_DATA_SIZE0, CNA_FC_DATA_SIZE0_DMA_WIDTH(7) | CNA_FC_DATA_SIZE0_DMA_HEIGHT(2));
         ...
         // EMIT(REG_CORE_DATAOUT_SIZE_0, CORE_DATAOUT_SIZE_0_DATAOUT_WIDTH(4));
         // EMIT(REG_CORE_DATAOUT_SIZE_1, CORE_DATAOUT_SIZE_1_DATAOUT_CHANNEL(15));
         // // [ffef2168] lsb 0801000000003030 - CORE Unknown
         // emit_raw(&regs, CORE | 0x1, 0x3030, 0);
         // EMIT(REG_DPU_FEATURE_MODE_CFG, DPU_FEATURE_MODE_CFG_BURST_LEN(15) | DPU_FEATURE_MODE_CFG_OUTPUT_MODE(2));
         // EMIT(REG_DPU_DATA_FORMAT, DPU_DATA_FORMAT_OUT_PRECISION(2) | DPU_DATA_FORMAT_IN_PRECISION(2) | DPU_DATA_FORMAT_PROC_PRECISION(2));
         // EMIT(REG_DPU_DST_BASE_ADDR, DPU_DST_BASE_ADDR_DST_BASE_ADDR(output_dma)+0xf0);
         // EMIT(REG_DPU_DST_SURF_STRIDE, DPU_DST_SURF_STRIDE_DST_SURF_STRIDE(20));
         // EMIT(REG_DPU_DATA_CUBE_WIDTH, DPU_DATA_CUBE_WIDTH_WIDTH(4));
         // EMIT(REG_DPU_DATA_CUBE_CHANNEL, DPU_DATA_CUBE_CHANNEL_ORIG_CHANNEL(5) | DPU_DATA_CUBE_CHANNEL_CHANNEL(15));
         // EMIT(REG_DPU_BS_CFG, DPU_BS_CFG_BS_RELU_BYPASS(1) | DPU_BS_CFG_BS_MUL_BYPASS(1) | DPU_BS_CFG_BS_ALU_BYPASS(1) | DPU_BS_CFG_BS_BYPASS(1));
         ...
         // EMIT(REG_DPU_OUT_CVT_SCALE, DPU_OUT_CVT_SCALE_FP32TOFP16_EN(1) | DPU_OUT_CVT_SCALE_OUT_CVT_SCALE(1));
         // EMIT(REG_DPU_SURFACE_ADD, DPU_SURFACE_ADD_SURF_ADD(40));
         // // [ffef2298] lsb 10010000000040c4 - DPU Unknown
         // emit_raw(&regs, 0x0 | 0x1, 0x40c4, 0);
         // EMIT(REG_PC_BASE_ADDRESS, PC_BASE_ADDRESS_PC_SOURCE_ADDR(0));
         // EMIT(REG_PC_REGISTER_AMOUNTS, PC_REGISTER_AMOUNTS_RESERVED_0(2) | PC_REGISTER_AMOUNTS_PC_DATA_AMOUNT(14));
         // EMIT(REG_PC_OPERATION_ENABLE, PC_OPERATION_ENABLE_RESERVED_0(6) | PC_OPERATION_ENABLE_OP_EN(1));
      ...
      else if (current_alu_algorithm == 12) { //CONV
         EMIT(REG_CNA_CBUF_CON0, CNA_CBUF_CON0_WEIGHT_BANK(11) | CNA_CBUF_CON0_DATA_BANK(1));
         EMIT(REG_CNA_DCOMP_REGNUM, 0x00000000);
         EMIT(REG_CNA_DCOMP_CTRL, 0x00000000);
         ...
         EMIT(REG_CNA_CONV_CON3, CNA_CONV_CON3_CONV_Y_STRIDE(1) | CNA_CONV_CON3_CONV_X_STRIDE(1));
         EMIT(REG_CNA_DATA_SIZE0, CNA_DATA_SIZE0_DATAIN_WIDTH(5) | CNA_DATA_SIZE0_DATAIN_HEIGHT(1));
         EMIT(REG_CNA_DATA_SIZE1, CNA_DATA_SIZE1_DATAIN_CHANNEL(8));
         EMIT(REG_CNA_DATA_SIZE2, CNA_DATA_SIZE2_DATAOUT_WIDTH(3));
         EMIT(REG_CNA_DATA_SIZE3, CNA_DATA_SIZE3_DATAOUT_ATOMICS(3));
         EMIT(REG_CNA_WEIGHT_SIZE0, 0x00000030);
         EMIT(REG_CNA_WEIGHT_SIZE1, CNA_WEIGHT_SIZE1_WEIGHT_BYTES_PER_KERNEL(48));
         EMIT(REG_CNA_WEIGHT_SIZE2, CNA_WEIGHT_SIZE2_WEIGHT_WIDTH(3) | CNA_WEIGHT_SIZE2_WEIGHT_HEIGHT(1) | CNA_WEIGHT_SIZE2_WEIGHT_KERNELS(1));
         EMIT(REG_CNA_CBUF_CON0, CNA_CBUF_CON0_WEIGHT_BANK(11) | CNA_CBUF_CON0_DATA_BANK(1));
         EMIT(REG_CNA_CBUF_CON1, CNA_CBUF_CON1_DATA_ENTRIES(2));
         ...
         EMIT(REG_CNA_FC_CON2, 0x00000000);
         EMIT(REG_CNA_DMA_CON0, CNA_DMA_CON0_WEIGHT_BURST_LEN(15) | CNA_DMA_CON0_DATA_BURST_LEN(15));
         EMIT(REG_CNA_DMA_CON1, CNA_DMA_CON1_LINE_STRIDE(20));
         EMIT(REG_CNA_DMA_CON2, CNA_DMA_CON2_SURF_STRIDE(268435444));
         EMIT(REG_CNA_FC_DATA_SIZE0, CNA_FC_DATA_SIZE0_DMA_WIDTH(5) | CNA_FC_DATA_SIZE0_DMA_HEIGHT(1));
         EMIT(REG_CNA_FC_DATA_SIZE1, CNA_FC_DATA_SIZE1_DMA_CHANNEL(8));
         EMIT(REG_CNA_DCOMP_CTRL, 0x00000000);
         EMIT(REG_CNA_DCOMP_REGNUM, 0x00000000);
         EMIT(REG_CNA_DCOMP_ADDR0, CNA_DCOMP_ADDR0_DECOMPRESS_ADDR0(weights_dma));
         ...
         EMIT(REG_CNA_PAD_CON1, 0x00000000);
         EMIT(REG_CORE_MISC_CFG, CORE_MISC_CFG_PROC_PRECISION(2));
         EMIT(REG_CORE_DATAOUT_SIZE_0, CORE_DATAOUT_SIZE_0_DATAOUT_WIDTH(2));
         EMIT(REG_CORE_DATAOUT_SIZE_1, CORE_DATAOUT_SIZE_1_DATAOUT_CHANNEL(2));
         EMIT(REG_CORE_CLIP_TRUNCATE, 0x00000000);
         
         // [ffef0a88] lsb 0801000000003030 - CORE Unknown
         emit_raw(&regs, CORE | 0x1, 0x3030, 0);

         EMIT(REG_DPU_FEATURE_MODE_CFG, DPU_FEATURE_MODE_CFG_BURST_LEN(2) | DPU_FEATURE_MODE_CFG_OUTPUT_MODE(2));
         EMIT(REG_DPU_DATA_FORMAT, DPU_DATA_FORMAT_OUT_PRECISION(2) | DPU_DATA_FORMAT_IN_PRECISION(2) | DPU_DATA_FORMAT_PROC_PRECISION(2));
         EMIT(REG_DPU_OFFSET_PEND, 0x00000000);
         EMIT(REG_DPU_DST_BASE_ADDR, DPU_DST_BASE_ADDR_DST_BASE_ADDR(output_dma));
         EMIT(REG_DPU_DST_SURF_STRIDE, DPU_DST_SURF_STRIDE_DST_SURF_STRIDE(1));
         EMIT(REG_DPU_DATA_CUBE_WIDTH, DPU_DATA_CUBE_WIDTH_WIDTH(2));
         EMIT(REG_DPU_DATA_CUBE_HEIGHT, 0x00000000);
         EMIT(REG_DPU_DATA_CUBE_NOTCH_ADDR, 0x00000000);
         EMIT(REG_DPU_DATA_CUBE_CHANNEL, DPU_DATA_CUBE_CHANNEL_CHANNEL(2));
         EMIT(REG_DPU_BS_CFG, DPU_BS_CFG_BS_RELU_BYPASS(1) | DPU_BS_CFG_BS_MUL_BYPASS(1) | DPU_BS_CFG_BS_ALU_BYPASS(1) | DPU_BS_CFG_BS_BYPASS(1));
         EMIT(REG_DPU_BS_ALU_CFG, 0x00000000);
         EMIT(REG_DPU_BS_MUL_CFG, 0x00000000);
         EMIT(REG_DPU_BS_RELUX_CMP_VALUE, 0x00000000);
         EMIT(REG_DPU_BS_OW_CFG, DPU_BS_OW_CFG_SIZE_E_2(0) | DPU_BS_OW_CFG_SIZE_E_1(0) | DPU_BS_OW_CFG_SIZE_E_0(0) | DPU_BS_OW_CFG_OD_BYPASS(1));
         EMIT(REG_DPU_BS_OW_OP, 0x00000000);
         EMIT(REG_DPU_WDMA_SIZE_0, DPU_WDMA_SIZE_0_CHANNEL_WDMA(2));
         EMIT(REG_DPU_WDMA_SIZE_1, DPU_WDMA_SIZE_1_WIDTH_WDMA(2));
         EMIT(REG_DPU_BN_CFG, DPU_BN_CFG_BN_RELU_BYPASS(1) | DPU_BN_CFG_BN_MUL_BYPASS(1) | DPU_BN_CFG_BN_ALU_BYPASS(1) | DPU_BN_CFG_BN_BYPASS(1));
         EMIT(REG_DPU_BN_ALU_CFG, 0x00000000);
         EMIT(REG_DPU_BN_MUL_CFG, 0x00000000);
         EMIT(REG_DPU_BN_RELUX_CMP_VALUE, 0x00000000);
...
struct MemHandles createRegCmd(int fd, size_t input_size, size_t weights_size, size_t output_size, uint32_t alu_algorithm)
{
   ...
   printf("task addr %zu %zu %zu %zu\n", tasks, tasks_dma, tasks_obj, tasks_handle);
   
   const size_t weights_aligned = (weights_size + 0x3f) & ~((size_t)0x3f);
   const size_t regcmd_padding = 0;
   // const size_t regcmd_padding = 0x1240;
   const size_t regcmd_reserved = 4096;
   const size_t regcmd_offset = weights_aligned + regcmd_padding;
   const size_t weights_alloc_size = regcmd_offset + regcmd_reserved;
   ...
   uint64_t reg_base_addr = weights_dma ;
   ...
   for (size_t i = 0; i < total_tasks; i++) {
      ...
      task->int_status = 0;
      task->regcfg_amount = (uint32_t)reg_task_lengths[i] - 3 ;
      task->regcfg_offset = 0;
      task->regcmd_addr = reg_base_addr + 0x240 ;//+ (uint64_t)reg_task_lengths[i] * sizeof(uint64_t);
      printf("check regcmd_addr=%p, reg_base_addr=%p, i=%d, reg_task_offsets[i]=%d, reg_task_lengths[i]=%d\n", task->regcmd_addr, reg_base_addr, i, reg_task_offsets[i], reg_task_lengths[i]);
   }
...
int submitTask(int fd, uint64_t tasks_obj, size_t task_count){
   ...
   struct rknpu_submit submit = {
      ...
      .timeout = 6000,
      .task_start = 0,
      .task_number = 3, //(uint32_t)task_count,
      .task_counter = 0,
      .priority = 0,
...
__fp16* float16_conv(__fp16* input, __fp16* kernel, uint32_t alu_algorithm, int input_size, int kernel_size)
{
   int fd = getDeviceFd();
   npu_reset(fd);
   ...
   size_t input_bytes   = (size_t)input_size * sizeof(__fp16);
   size_t kernel_bytes  = (size_t)kernel_size * sizeof(__fp16);
...
__fp16* float16_conv2d(__fp16* input, __fp16* kernel, uint32_t alu_algorithm, int input_size, int kernel_size)
{
   ...
   const int conv_batch = 1;
   const int conv_in_channels = 3;
   const int conv_in_height = 5;
   const int conv_in_width = 7;
   const int conv_out_channels = 6;
   const int conv_kernel_h = 2;
   const int conv_kernel_w = 3;
   const int conv_align_c = 8;
   const int conv_align_out_c = 8;
   const int conv_width_stride = 8;
   const int conv_out_width_stride = 5;
   ...
   if (use_packed) {
      pack_conv_weights_fp16(kernel_fp16, kernel,
         conv_out_channels, conv_in_channels, conv_kernel_h, conv_kernel_w, conv_align_c, conv_align_out_c);
   ...
   mem_destroy(fd, handles.input, handles.input_dma);
