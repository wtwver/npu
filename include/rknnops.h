...
void regcmd_helper(input_dma, weights_dma, output_dma){
   ...
   for (int i = 0; i < 1; ++i) {
      if (current_alu_algorithm == 13) { //CONV2d
         ...
         emit_raw(&regs, 0x0 | 0x1, 0x40c4, 0);
         EMIT(REG_PC_BASE_ADDRESS, PC_BASE_ADDRESS_PC_SOURCE_ADDR((weights_dma+0x1400)/16));
         ...
         emit_raw(&regs, 0x0 | 0x1, 0x40c4, 0);
         EMIT(REG_PC_BASE_ADDRESS, PC_BASE_ADDRESS_PC_SOURCE_ADDR(268425564));
         EMIT(REG_PC_REGISTER_AMOUNTS, PC_REGISTER_AMOUNTS_RESERVED_0(1) | PC_REGISTER_AMOUNTS_PC_DATA_AMOUNT(14));
         EMIT(REG_PC_OPERATION_ENABLE, PC_OPERATION_ENABLE_RESERVED_0(6) | PC_OPERATION_ENABLE_OP_EN(1));
         ...
         EMIT(REG_PPU_MISC_CTRL, PPU_MISC_CTRL_BURST_LEN(3));
         EMIT(REG_PPU_RDMA_RDMA_CUBE_IN_CHANNEL, PPU_RDMA_RDMA_CUBE_IN_CHANNEL_CUBE_IN_CHANNEL(31));
         EMIT(REG_PPU_RDMA_RDMA_SRC_BASE_ADDR, 0xfffd8640);
         EMIT(REG_PPU_RDMA_RDMA_SRC_LINE_STRIDE, PPU_RDMA_RDMA_SRC_LINE_STRIDE_SRC_LINE_STRIDE(1));
         EMIT(REG_PPU_RDMA_RDMA_SRC_SURF_STRIDE, PPU_RDMA_RDMA_SRC_SURF_STRIDE_SRC_SURF_STRIDE(1));
         EMIT(REG_PPU_RDMA_RDMA_DATA_FORMAT, PPU_RDMA_RDMA_DATA_FORMAT_IN_PRECISION(1));
         EMIT(REG_PC_OPERATION_ENABLE, PC_OPERATION_ENABLE_RESERVED_0(48));
         ...
         emit_raw(&regs, 0x0 | 0x1, 0x40c4, 0);
         EMIT(REG_PC_BASE_ADDRESS, PC_BASE_ADDRESS_PC_SOURCE_ADDR(268425636));
         ...
         emit_raw(&regs, 0x0 | 0x1, 0x40c4, 0);
         EMIT(REG_PC_BASE_ADDRESS, PC_BASE_ADDRESS_PC_SOURCE_ADDR(268425708));
         EMIT(REG_PC_REGISTER_AMOUNTS, PC_REGISTER_AMOUNTS_RESERVED_0(2) | PC_REGISTER_AMOUNTS_PC_DATA_AMOUNT(14));
         EMIT(REG_PC_OPERATION_ENABLE, PC_OPERATION_ENABLE_RESERVED_0(6) | PC_OPERATION_ENABLE_OP_EN(1));
         ...
         emit_raw(&regs, 0x0 | 0x1, 0x40c4, 0);
         EMIT(REG_PC_BASE_ADDRESS, PC_BASE_ADDRESS_PC_SOURCE_ADDR(268425780));
         EMIT(REG_PC_REGISTER_AMOUNTS, PC_REGISTER_AMOUNTS_RESERVED_0(2) | PC_REGISTER_AMOUNTS_PC_DATA_AMOUNT(14));
         EMIT(REG_PC_OPERATION_ENABLE, PC_OPERATION_ENABLE_RESERVED_0(6) | PC_OPERATION_ENABLE_OP_EN(1));
...
struct MemHandles createRegCmd(int fd, size_t input_size, size_t weights_size, size_t output_size, uint32_t alu_algorithm)
{
   ...
   uint64_t *regcmd         = mem_allocate(fd, 4096, &regcmd_dma, &regcmd_obj, 0, &regcmd_handle);
   ...
   if (
      ...
      create_flink_name(fd, regcmd_handle, &regcmd_flink) < 0 ||
   ...
   if (reg_bytes > 4096) {
      printf("Warning: regcfg size %zu exceeds allocated buffer, truncating\n", reg_bytes);
      reg_bytes = 4096;
   }
   memset(regcmd, 0, 4096);
   memcpy(regcmd, regs.data, reg_bytes);
   ...
   tasks[0].regcfg_amount = (uint32_t)regs.size;
   // hardcoded for matmul test
   // tasks[0].regcfg_amount = 104;
   printf("tasks[0].regcfg_amount %zu", tasks[0].regcfg_amount) ;
